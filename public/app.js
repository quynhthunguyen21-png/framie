...
function setAuth(x){if(S.user&&!S.user.guest&&x.user&&S.user.id!==x.user.id)resetIdentityState();S.user=x.user;S.token=x.token;localStorage.setItem('framie_user',JSON.stringify(S.user));localStorage.setItem('framie_token',S.token);persist()}
...
function bindBankQr(){
  ...
  if(!summary||!paymentInputs.length)return;
  const refresh=()=>{
    const selected=$('input[name="payment"]:checked')?.value;
    const src=selected==='bank'?bankQrForTotal(calcCart().total):'';
    let qr=$('#bank-qr');
    if(!src){qr?.remove();return}
    if(!qr){qr=document.createElement('div');qr.id='bank-qr';qr.className='bank-qr';summary.appendChild(qr)}
    qr.innerHTML=`<span class="eyebrow">QUÉT MÃ ĐỂ CHUYỂN KHOẢN</span><img src="${src}" alt="Mã QR thanh toán chuyển khoản">`;
  };
  paymentInputs.forEach(input=>input.addEventListener('change',refresh));
  refresh();
}
function shell(content,{wide=false,plain=false}={}){
  ...
  const nav=[['/about','Về Framie'],['/shop','Cửa hàng'],['/setup','Trạm thiết lập'],['/blog','Blog'],['/contact','Liên hệ']];
  ...
  if(location.pathname==='/checkout') bindBankQr();
...
function calcCart(){const sub=S.cart.reduce((a,x)=>a+(x.price*x.qty),0);const discount=S.coupon?Math.round(sub*(S.coupon.discount||0)):0;const shipping=sub===0?0:30000;return {sub,discount,shipping,total:Math.max(0,sub-discount+shipping)}}
function bankQrForTotal(total){return {179000:'/qr1.JPG',279000:'/qr2.JPG',379000:'/qr3.JPG'}[total]||''}
...
route=function(){
  ...
  if(location.pathname==='/login'&&S.step===7)localStorage.setItem('framie_after_auth','/setup');
  if(location.pathname==='/auth/google/complete'&&S.step===7){
    const data=new URLSearchParams(location.search).get('data');
    try{const payload=JSON.parse(base64UrlDecode(data));setAuth(payload);localStorage.removeItem('framie_after_auth');navigate('/setup');return}catch{}
  }
  routeWithGuestSetup();
};
