...
const plans=[
  {code:'basic',number:'01',name:'Framie Basic',price:99000,description:'Một chiếc khung tối giản cho một khoảnh khắc chỉ hai người hiểu.',features:['Tối đa 1 ảnh','Text mặt khung','NFC landing cơ bản'],note:'Hợp để đánh dấu sinh nhật, ngày đầu yêu hoặc một lời nhắn riêng.'},
  {code:'memory',number:'02',name:'Framie Memory',price:199000,description:'Không gian đủ đầy để kể một câu chuyện tình yêu bằng ảnh và giọng nói.',features:['Tối đa 5 ảnh','Ghi âm / audio','NFC landing nâng cao'],note:'Dành cho kỷ niệm yêu nhau, yêu xa, sinh nhật hoặc ngày đặc biệt.',popular:true},
  {code:'standard',number:'03',name:'Framie Standard',price:299000,description:'Một trải nghiệm Phygital trọn vẹn cho những cột mốc lớn của hai người.',features:['Thiết kế riêng không giới hạn','Album ảnh số','Audio + video đa tầng','Quản trị bảo mật cao cấp'],note:'Dành cho lễ cầu hôn, quà cưới, kỷ niệm lớn và những dịp thật riêng.'}
];
...
function useTemplate(id){const t=templates.find(x=>x.id===id);if(!t)return;S.design={plan:t.plan,frame:t.frame,color:t.color,bg:t.bg,size:t.size,elements:JSON.parse(JSON.stringify(t.elements))};S.nfc=defaultNfc();S.selected=null;S.nfcSelected=null;S.loadedDesignId=null;S.productPlan=t.plan;S.step=3;persist();location.hash='/setup'}
...
function signout(){resetIdentityState();S.user=null;S.token='';localStorage.removeItem('framie_user');localStorage.removeItem('framie_token');persist();location.hash='/'}
...
function md2html(md) {
  return md
    ...
    .map(block => {
      block = block.trim();
      if (!block) return '';
      if (/^<(h[1-6]|ul|ol|blockquote|hr|li)/.test(block)) return block;
...
function googleAuthComplete(){const {q}=parseRoute();const data=q.get('data');try{if(!data)throw new Error('missing');const payload=JSON.parse(base64UrlDecode(data));setAuth(payload);location.hash='/dashboard'}catch{location.hash='/login?error=google'}}
...
function renderSetup(){
  ...
  $('#next').onclick=async()=>{if(S.step===2){privacy();return}if(S.step===7){const d=await saveDesign('ready');if(!d)return;if(S.designHasOrder){alert('Đã lưu thay đổi cho thiết kế của bạn.');location.hash='/dashboard';return}location.hash='/checkout?design='+d.id;return}S.step++;persist();renderSetup()};
...
async function dashboard(){
  refreshAuth();if(!S.user){location.hash='/login';return}
  ...
  if(dsRes.status===401||osRes.status===401||anRes.status===401){signout();location.hash='/login';return}
...
window.addEventListener('hashchange',()=>{window.scrollTo({top:0,left:0,behavior:'instant'});route()});route();
