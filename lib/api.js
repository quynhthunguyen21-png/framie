...
const catalog = {
  plans: [
    { code: 'basic', number: '01', name: 'Framie Basic', price: 149000, description: 'Khung tranh NFC tinh gọn cho một kỷ niệm thật đẹp.', features: ['Tối đa 1 ảnh', 'Text mặt khung', 'NFC landing cơ bản'] },
    { code: 'memory', number: '02', name: 'Framie Memory', price: 249000, description: 'Không gian cho ảnh, âm thanh và một câu chuyện nhiều lớp.', features: ['Tối đa 5 ảnh', 'File thu âm / audio', 'NFC landing nâng cao'], popular: true },
    { code: 'standard', number: '03', name: 'Framie Standard', price: 349000, description: 'Trải nghiệm custom trọn vẹn cho món quà đặc biệt.', features: ['Thiết kế riêng không giới hạn', 'Album ảnh số', 'Audio + video đa tầng', 'Quản trị bảo mật cao cấp'] }
  ],
...
export async function api(req, res) {
  ...
  if (p === '/api/checkout' && req.method === 'POST') {
    const user = await who(req); if (!auth(res, user)) return; const b = await body(req);
    const requestedItems = Array.isArray(b.items) ? b.items : [];
    const items = requestedItems.map(item => {
      const plan = catalog.plans.find(candidate => candidate.code === item.code);
      const qty = Math.max(1, Math.floor(Number(item.qty) || 1));
      return plan ? { code: plan.code, name: plan.name, price: plan.price, qty } : null;
    }).filter(Boolean);
    if (!items.length) return json(res, 400, { message: 'Giỏ hàng không có sản phẩm hợp lệ.' });
    const subtotal = items.reduce((sum, item) => sum + item.price * item.qty, 0);
    const couponRates = { FRAMIE10: 0.10, MEMORY20: 0.20, WELCOME50: 0.50 };
    const couponCode = String(b.coupon?.code || '').trim().toUpperCase();
    const discountRate = couponCode ? couponRates[couponCode] : 0;
    if (couponCode && discountRate === undefined) return json(res, 400, { message: 'Mã giảm giá không hợp lệ.' });
    const discount = Math.round(subtotal * discountRate);
    const shipping = subtotal >= 300000 ? 0 : 30000;
    const total = Math.max(0, subtotal - discount + shipping);
    const coupon = couponCode ? { code: couponCode, discount: discountRate } : null;
    const { rows } = await pool.query(
      ...
      [user.id, b.designId || null, JSON.stringify(items), total, coupon ? JSON.stringify(coupon) : null, b.paymentMethod || 'bank', b.paymentMethod === 'cod' ? 'pending' : 'prototype_pending', JSON.stringify(b.shipping || {})]
    );
    const id = rows[0].id;
